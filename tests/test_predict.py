...
from app import predictDB, get_metadata
...
class TestPredict(TestCase):
    ...
    def test_model_predict_1065_array(self):
        ...
        smiles_list.append(
            "CCCCCCCc1ccccc1")  # one of neighbors for test set doesnt have matching dtxsid for the dtxcid (not in dsstox_records table)
        ...
        smiles_list.append(
            "[H][C@]12[C@@H](Cl)[C@H](Cl)[C@](C(Cl)Cl)([C@@H](Cl)[C@@H]1Cl)C2(C)C(Cl)Cl")  # DTXCID001783033
        smiles_list.append(
            "[H][C@]12O[C@@]1([H])[C@@]1([H])[C@@]([H])([C@H]2Cl)[C@@]2(Cl)C(Cl)=C(Cl)[C@]1(Cl)C2(Cl)Cl")  # DTXCID501783733

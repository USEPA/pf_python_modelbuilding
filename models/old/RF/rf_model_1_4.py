...
from sklearn.model_selection import KFold, cross_val_score, train_test_split, RandomizedSearchCV, cross_validate, GridSearchCV
...
class Model:
    ...
    def getModel(self):
        ...
        if self.is_binary:
            self.rfr = RandomForestClassifier(n_estimators=self.n_estimators, random_state=42, n_jobs=self.n_threads,
                                              min_impurity_decrease=self.min_impurity_decrease, max_samples=self.max_samples)
        else:
    ...
    def build_model(self):
        ...
        import pickle
        print(pickle.dumps(self).__sizeof__())


        # pickle.dump(rfr, open("rfr.p", "wb"))
        return self
    
    def build_model_with_grid_search_with_preselected_descriptors(self, descriptor_names):
        ...
        optimizer = GridSearchCV(self.rfr, self.hyper_parameter_grid, n_jobs=self.n_threads, return_train_score=True, cv=kfold_splitter, verbose=3)
    ...
    def build_model_with_grid_search(self):

...
class Model:
    ...
    def build_model(self):
        ...
        optimizer = GridSearchCV(self.model, {'estimator__C': list([10**x for x in range(-3,4)]), 'estimator__gamma':list([10**x for x in range(-3,4)])}, n_jobs=self.n_threads)
    ...
    def build_model_with_preselected_descriptors(self, descriptor_names):
        ...
        optimizer = GridSearchCV(self.model, {'estimator__C': np.arange(-3,4, 0.5), 'estimator__gamma':self.gamma_space})
        optimizer.fit(train_features, train_labels)

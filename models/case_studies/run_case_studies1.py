...
def calculate_stats_for_subset(dataset_name, dataset_name_subset, append_to_models_folder, run_folder):
    import os
    ...
    df_smiles_test_set_rifm = fetch_test_set_qsar_smiles(session, dataset_name_subset)
    
    df_pred_in_rifm_test = df_pred.merge(
        df_smiles_test_set_rifm[["canon_qsar_smiles"]].drop_duplicates(), 
        on="canon_qsar_smiles", 
        how="inner")
    test_stats = calculate_binary_statistics(df_pred_in_rifm_test, 0.5, "_Test")
    
    print(run_folder)
    print('RIFM test chemical stats', json.dumps(test_stats,indent=4))
    # print(df_pred_in_rifm_test.shape[0])
    
    df_fragrances_list = getQsarSmilesFromFragranceSpreadsheet()
    df_fragrances_list_only = df_fragrances_list[~df_fragrances_list['canon_qsar_smiles'].isin(df_smiles_test_set_rifm['canon_qsar_smiles'])]
    
    df_pred_fragrances = df_pred.merge(
        df_fragrances_list_only[["canon_qsar_smiles"]].drop_duplicates(), 
        on="canon_qsar_smiles", 
        how="inner")
    test_stats = calculate_binary_statistics(df_pred_fragrances, 0.5, "_Test")
...
def run_biodeg_301F():
    ...
    # print(df_pred_in_rifm_test)
    # print(smiles_test_set_rifm)
    ...
    # run_folder='xgb_WebTEST-default_fs=False'
    # run_folder="gcm_WebTEST-default_fs=False"
    # run_folder='rf_WebTEST-default_fs=False'
    # run_folder='rf_WebTEST-default_fs=True'
    # run_folder='xgb_WebTEST-default_fs=True'
    # calculate_stats_for_subset(dataset_name, dataset_name_subset, append_to_models_folder, run_folder)
    
    folder = Path(os.getenv("PROJECT_ROOT")) / "data" / "models" / dataset_name
    
    for entry in folder.iterdir():
        if entry.is_dir():
            # entry is already a Path pointing to the subfolder
            print("Subfolder:", entry.name)
            calculate_stats_for_subset(dataset_name, dataset_name_subset, append_to_models_folder, entry.name)
    
    
    #TODO get stats for tony fragrance compounds that arent in RIFM dataset

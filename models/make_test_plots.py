...
def getBin(exps, property_name,unit_name):
    ...
    if property_name == 'Density' or property_name == 'Viscosity':
        bins = numpy.linspace(minVal, maxVal, (maxVal - minVal) * 2 + 1)
...
def getBin2(expsTrain,expsTest, property_name, unit_name):
    ...
    if property_name == 'Viscosity':
        ...
        bins = numpy.linspace(0.5, 4, 15)

    elif property_name == 'Liquid Chromatography Retention Time':

        bins = numpy.linspace(0, 50, 11)
        ...
        # bins = int((maxVal - minVal) / 10)
        # bins = range(minVal,maxVal, )
        bins = numpy.linspace(minVal, maxVal, int((maxVal - minVal) / 10)+1)
        ...
        if minVal < 0:
            minVal = 0
        # bins = int((maxVal - minVal) / 100)
        bins = numpy.linspace(minVal, maxVal, (int((maxVal - minVal) / 25))+1)

    elif '°C' in unit_name:
        maxVal = round(maxVal, -2) + 50
        minVal = round(minVal, -2) - 50
        # bins = int((maxVal - minVal) / 100)
        bins = numpy.linspace(minVal, maxVal, (int((maxVal - minVal) / 50))+1)
        ...
        if nbins < 12:
            bins = numpy.linspace(minVal, maxVal, 2*(maxVal-minVal)+1)
        else:
            bins = range(minVal, maxVal)
    else:

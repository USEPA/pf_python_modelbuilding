...
class ExpDataGetter:
    ...
    def test_get_exp_data():
        ...
        path_segments = [PROJECT_ROOT, "data", "reports", "property_records.json"]
        ...
        path_segments = [PROJECT_ROOT, "data", "reports", "property_records.html"]
        temp_file_path = os.path.join(*path_segments)
    ...
    @staticmethod
    def _getSqlPropertyValuesForDataset():
    ...
    @staticmethod
    def get_mapped_records_by_source(dataset_name):
        
        session = getSession()
        df_mapped = ExpDataGetter.getMappedPropertyValues(session, dataset_name)
        
        print_first_row(df_mapped)
        
        counts_ps = (
            df_mapped.loc[df_mapped['public_source_name'].notna(), 'public_source_name'].value_counts())
    
        # print(counts_ps)
        
        counts_ls = (
            df_mapped.loc[df_mapped['public_source_name'].isna(), 'literature_source_name']
                    .value_counts()
        )
        
        combined = counts_ps.add(counts_ls, fill_value=0).astype(int).sort_values(ascending=False)
        
        
            # Sum all categories except the TOTAL row
        other_sum = pd.to_numeric(combined).sum()
        total_rows = len(df_mapped)
        
        # print(combined)
    
    
        combined_with_total = pd.concat([combined, pd.Series({'TOTAL': total_rows})])
        print(combined_with_total)
        
        print(f"Sum of categories: {other_sum}")
        print(f"TOTAL: {total_rows}")
        print(f"Matches? {other_sum == total_rows}")
        
        # If it doesn't match, show how many rows are unrepresented (both names missing)
        if other_sum != total_rows:
            missing_both = (df_mapped['public_source_name'].isna() &
                            df_mapped['literature_source_name'].isna()).sum()
            print(f"Rows with both public_source_name and literature_source_name missing: {missing_both}")
            print(f"Difference (TOTAL - sum): {total_rows - other_sum}")
    ...
    def getMappedPropertyValues(session, dataset_name):
        ...
        sql = ExpDataGetter._getSqlPropertyValuesForDataset()
        
        df_pv = pd.read_sql(sql, con=session.get_bind(), params={"dataset_name": dataset_name})
    ...
    def _getSqlPropertyValues(self):
    ...
    def get_raw_exp_data(self, session, datasetName, qsarSmiles):
        """
        TODO: Can this be replaced by getMappedPropertyValues?
        """
        
        try:
            
            # TODO: need to sort the propertyvalues and mark the ones that were used in the qsar property value
            resultsPropertyValues = session.execute(self._getSqlPropertyValues(), {"datasetName": datasetName, "qsarSmiles": qsarSmiles})
            df_pv = pd.DataFrame(resultsPropertyValues.mappings().all())
                                            
            # Get the parameters separately to make the query manageable
            resultsParameters = session.execute(self._getSqlParameters(), {"datasetName": datasetName, "qsarSmiles": qsarSmiles})
...
class SplittingGenerator:
    ...
    def create_splittings(datasetName, descriptorSetName = 'WebTEST-default'):
        ...
        SplittingGenerator.preview_and_delete_splittings_by_dataset(
        ...
        # all in one:
        SplittingGenerator.make_representative_and_inner_cv_splits(datasetName, descriptorSetName, remove_log_p_descriptors, user, 
...
if __name__ == '__main__':
    ...
    ExpDataGetter.test_get_exp_data()
